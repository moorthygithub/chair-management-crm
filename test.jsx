...
const BomDialog = ({ open, onClose, bomId }) => {
  ...
  useEffect(() => {
    if (isEditMode && open) fetchBomDetails();
  ...
  /* ---------------- Add / Remove Sub ---------------- */
  const handleAddSub = () => {
    setFormData((p) => ({ ...p, subs: [...p.subs, { ...initialSub }] }));
  };

  const handleRemoveSubClick = (index) => {
    const sub = formData.subs[index];
    if (sub.id) {
      // Open confirmation dialog if sub has an id
      setDeleteIndex(index);
      setDeleteId(sub.id);
      setDeleteDialogOpen(true);
    } else {
      // Remove immediately if new sub
  ...
  const handleConfirmDeleteSub = async () => {
    ...
    try {
      ...
      if (res?.code === 200 || res?.code === 201) {
        toast.success(res?.msg || "Sub-component deleted");
        handleRemoveSub(deleteIndex);
      } else {
        toast.error(res?.msg || "Failed to delete sub-component");
      }
    } catch (err) {
  ...
  const handleSubChange = (index, field, value) => {
    ...
    setFormData({ ...formData, subs: updated });
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res?.code === 201) {
        ...
        toast.error(res?.message || "Failed to save BOM");
      }
    } catch (err) {
      toast.error(err?.message || "Something went wrong");
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
          ...
          <div className="space-y-6">
            ...
            <div className="mx-1">
              ...
              {errors.bom_product_id && (
                <p className="text-sm text-red-500">{errors.bom_product_id}</p>
              )}
            </div>
                      ...
                      <TableRow key={index}>
                        <TableCell>
                          <Select
                            ...
                            onValueChange={(v) =>
                              handleSubChange(index, "bom_sub_component_id", v)
                            }
                          >
                            <SelectTrigger>
      ...
      {/* Delete Sub-component Alert Dialog */}
