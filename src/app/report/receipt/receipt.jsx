...
const Recepit = () => {
  ...
  const handlePrintPdf = useReactToPrint({
    content: () => {
      if (!downloadDonor.receipt_from_date || !viewType) {
        toast.warning("Please select a recepit and view type before printing!");
        return null;
      }
      return componentRef.current;
    },
    documentTitle: viewType == "Recepit_Report",
    pageStyle: `
  ...
  const handleSavePDF = () => {
    if (!downloadDonor.receipt_from_date || !viewType) {
      toast.warning("Please select a recepit and view type before saving PDF.");
      return;
    }
  ...
  const handleDownload = async (e) => {
    ...
    if (!downloadDonor.receipt_from_date) {
      toast.warning("Please select a recepit before downloading.");
      return;
    }
    ...
    try {
      ...
      if (!res) {
        toast.warning("No data found for the selected recepit.");
        return;
      }
      ...
      link.href = url;
      link.setAttribute("download", "recepit_summary.csv");
      document.body.appendChild(link);
      link.click();
      link.remove();

      toast.success("Recepit Summary Downloaded Successfully!");
    } catch (err) {
      console.error("Error downloading Recepit summary:", err);
